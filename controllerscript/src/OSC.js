...
function OSCRegisterCallback(listenToHost, listenToPort, fn) {
    host.addDatagramPacketObserver (listenToHost, listenToPort, doObject (this, function (data)
    {
        var msg = new OSCMessage ();
        ...
        {
            fn(messages[i].address, messages[i].values);
        }
    }));
}

function OSCSendMessage(sendToHost, sendToPort, path, value) {

    var convertBooleanToInt = function (val)
    {
        return typeof (val) == 'boolean' ? (val ? 1 : 0) : val;
    };

    //println("send message to " + sendToHost + ":" + sendToPort);
    
    // Convert boolean values to integer for client compatibility
    if (value instanceof Array)
    {
        for (var i = 0; i < value.length; i++)
            value[i] = convertBooleanToInt (value[i]);
    }
    else {
        value = convertBooleanToInt (value);
    }

    var msg = new OSCMessage();
    msg.init(path, value);
    var data = new OSCMessage().buildBundle([msg.build()], 1000);
    host.sendDatagramPacket(sendToHost, sendToPort, data);
}

function OSCSendMessages(sendToHost, sendToPort, pathValueArray) {
    ...
    var msgs = []
    for(var i in pathValueArray) {
        var path = pathValueArray[i].path;
        var value = pathValueArray[i].value;
    ...
    while(msgs.length > 0) {
        var data = new OSCMessage().buildBundle(msgs, 1000);
        host.sendDatagramPacket(sendToHost, sendToPort, data);
    }
}
